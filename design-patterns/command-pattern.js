...
class FieldEditor {
    ...
    cleanCommandsByProp(propName, propValue) {
        this.undoHistory = this.undoHistory.filter(command => {
            return command[propName] === propValue;
        });
        this.redoHistory = this.redoHistory.filter(command => {
            return command[propName] === propValue;
        });
    }
    getFields() {
